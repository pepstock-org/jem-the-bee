...
/**
 * Main panel of job INPUT queue manager. Shows the list of jobs in inut queue with the possibilities to act on them. Furthermore allows to inspect
 * the job to see JCL and general information.
 * 
 * @author Andrea "Stock" Stocchero
 * 
 */
public class Input extends BasePanel<Job> implements SearchListener, UpdateListener<Job> {

	/**
	 * Constructs all UI 
	 */
	public Input() {
		super(new TableContainer<Job>(new InputTable(true)),
				new CommandPanel<Job>(new JobsSearcher(PreferencesKeys.JOB_SEARCH_INPUT), new InputActions()));
	...
	public void search(final String jobsFilter) {
	    ...
	    scheduler.scheduleDeferred(new ScheduledCommand() {
			...
			@Override
			public void execute() {
				Services.QUEUES_MANAGER.getInputQueue(jobsFilter, 
	...
	public void inspect(final Job job) {
	    ...
	    scheduler.scheduleDeferred(new ScheduledCommand() {
			...
			public void execute() {
				// asks for JCL
				Services.QUEUES_MANAGER.getJcl(job, Queues.INPUT_QUEUE, new GetJclAsyncCallback(job));
	...
	public void update(final Job job) {
		if (job != null){
		    ...
		    scheduler.scheduleDeferred(new ScheduledCommand() {
				...
				public void execute() {
					// asks for JCL
					Services.QUEUES_MANAGER.update(job, Queues.INPUT_QUEUE, new UpdateJobAsyncCallback());
				}
		    });
		} else {
	...
	private class UpdateJobAsyncCallback extends ServiceAsyncCallback<Boolean> {
		
		@Override
		public void onJemFailure(Throwable caught) {
			new Toast(MessageLevel.ERROR, caught.getMessage(), "Update JOB error!").show();
		}
