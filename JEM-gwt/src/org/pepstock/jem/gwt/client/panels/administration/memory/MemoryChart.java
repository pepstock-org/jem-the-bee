 ...
 * @version 2.0
 */
public class MemoryChart extends AbstractPieChart {
	...
	public String getProvidedLabel(PieData item, ValueProvider<? super PieData, ? extends Number> valueProvider) {
		return item.getKey()+": "+NumberFormat.getFormat("##0.00 %").format((double)item.getPercent())+", "+NumberFormat.getFormat("###,###,##0 MB").format((double)item.getValue()/1024/1024);
	}
