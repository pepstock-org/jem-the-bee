 ...
 *
 */
public class ViewClusterCommand extends CommandExecutor {
    ...
    public void execute(final NodeInfoBean node, final ResultPanel resultPanel) {
	    ...
	    scheduler.scheduleDeferred(new ScheduledCommand() {
			...
			@Override
			public void execute() {
				Services.NODES_MANAGER.displayCluster(node, new DisplayClusterAsyncCallback(resultPanel));
			}
	    });
    }
    
    private static class DisplayClusterAsyncCallback extends ServiceAsyncCallback<String> {
    	
    	private final ResultPanel resultPanel;
    	
    	public DisplayClusterAsyncCallback(final ResultPanel resultPanel) {
    		this.resultPanel = resultPanel;
    	}

		@Override
		public void onJemFailure(Throwable caught) {
			new Toast(MessageLevel.ERROR, caught.getMessage(), "View cluster error!").show();
		}
		...
		public void onJemSuccess(String result) {
			...
			if (result != null){
				...
				new Toast(MessageLevel.WARNING, "View cluster result is null. <br>Please have a look to JEM node log", "View cluster  warning!").show();
			}
		}
