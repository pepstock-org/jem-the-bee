...
public class CommonResourceDefinitionsManager extends DefaultService {
	 ...
	 * @throws ServiceMessageException 
	 */
	public ResourceDescriptor getDescriptorOf(String resourceName) throws ServiceMessageException  {
		String permission = Permissions.RESOURCES_READ;
		checkAuthorization(new StringPermission(permission));
	
		DistributedTaskExecutor<ResourceDescriptor> task = new DistributedTaskExecutor<ResourceDescriptor>(new GetResourceDescriptor(resourceName), getMember());
		return task.getResult();
	}
