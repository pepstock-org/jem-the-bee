...
*/
package org.pepstock.jem.jbpm.tasks.utilities.sort;
...
public class DefaultComparator implements Comparator<String>, Serializable {
	...
	public DefaultComparator(){
		...
		try {
			...
			LogAppl.getInstance().emit(JBpmMessage.JEMM044W, e, e.getMessage());
		} catch (ParseException e) {
			LogAppl.getInstance().emit(JBpmMessage.JEMM058W, e, e.getMessage());
		} catch (IOException e) {
			LogAppl.getInstance().emit(JBpmMessage.JEMM058W, e, e.getMessage());
		} 
	}
	...
	private SingleComparator createSingleComparator(Object[] result) throws ParseException{
		...
		if (result[ELEMENT_1] instanceof Number){
			Number n = (Number) result[ELEMENT_1];
