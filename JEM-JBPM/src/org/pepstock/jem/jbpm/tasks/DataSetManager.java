...
public class DataSetManager {
	 ...
	 * @throws JBpmException if disposition is not compliant with reference
	 */
	static void loadReferences(DataDescriptionImpl ddImpl, DataSet ds) throws JBpmException {
		// checks if reference
		if (ds.isReference()) {
			...
			if (ddImpl.getDisposition().equalsIgnoreCase(Disposition.NEW)){
				throw new JBpmException(JBpmMessage.JEMM009E, ddImpl.getName(), ddImpl.getDisposition(), ds.toString());
			}
			// container doesn't have the reference, file not previously
			// defined, so exception
			if (!container.hasDataDescription(ds.getName())) {
				throw new JBpmException(JBpmMessage.JEMM010E, ddImpl.getName(), ds.getName());
			}
			...
			for (DataSetImpl dataset : referback.getDatasets()) {
				...
				if (!file.exists()) {
					throw new JBpmException(JBpmMessage.JEMM011E, ddImpl.getName(), ddImpl.getDisposition(), ds.toString(), new FileNotFoundException(file.getAbsolutePath()));
				}
	...
	static void createDataSetImpl(DataDescriptionImpl ddImpl, DataSet ds, Task item) throws IOException, JBpmException {
		...
		if (ds.isReference()) {
			...
			if (ds.getName() == null){
				throw new JBpmException(JBpmMessage.JEMM014E, ddImpl.getName(), ddImpl.getDisposition());
			}
			...
			String value = VariableSubstituter.substitute(ds.getText().toString(), Variables.getInstance().getProperties());
			IOUtils.write(value, fos, CharSet.DEFAULT_CHARSET_NAME);
			fos.flush();
			fos.close();
			...
			if (ddImpl.getDisposition().equalsIgnoreCase(Disposition.NEW)) {
				...
				if (!parent.exists()){
					...
					if (!isCreated){
						throw new JBpmException(NodeMessage.JEMC153E, parent.getAbsolutePath());
					}
				}
			} else {
