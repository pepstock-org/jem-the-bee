 ...
 * 
 */
public class RolesTask extends TestCase {
	...
	public void testRole() throws Exception {
		Future<SubmitResult> future = JemTestManager.getSharedInstance()
				.submit(getJcl("TEST_ANTUTILS_CREATE_ROLE.xml"), "ant", true,
						false);
		SubmitResult sr = future.get();
		assertEquals(sr.getRc(), 0);
		future = JemTestManager.getSharedInstance().submit(
				getJcl("TEST_ANTUTILS_REVOKE_PERMISSION.xml"), "ant", true,
				false);
		sr = future.get();
	}

	private String getJcl(String name) {
		return this.getClass().getResource("jcls/role/" + name).toString();
	}
}
