...
public class Clean extends TestCase{
	...
	public void testCleanJobs() throws Exception {
		Future<SubmitResult>future = JemTestManager.getSharedInstance()
				.submit(getJcl("TEST_JPPF_CLEAN_JOBS.xml"), "ant", true,
						false);
	...
	public void testCleanDataset() throws Exception {
		Future<SubmitResult> future = JemTestManager.getSharedInstance()
				.submit(getJcl("TEST_JPPF_CLEAN_DATASETS.xml"), "ant", true,
						false);
		SubmitResult sr = future.get();
		assertEquals(sr.getRc(), 0);
	}
	 ...
	 * @throws Exception
	 */
	public void testCleanJDBC() throws Exception {
		Future<SubmitResult> future = JemTestManager.getSharedInstance()
				.submit(getJcl("TEST_JPPF_REMOVE_JEM_JDBC_DATA_SOURCE.xml"), "ant", true,
						false);
		future.get();
	}
	...
	public void testCleanResources() throws Exception {
		Future<SubmitResult> future = JemTestManager.getSharedInstance()
				.submit(getJcl("TEST_JPPF_RESOURCE_REMOVE.xml"), "ant", true,
						false);
		SubmitResult sr = future.get();
		assertEquals(sr.getRc(), 0);
	}
