...
public class Grant extends Command {
	...
	public static final String COMMAND_KEYWORD = "GRANT";
	
	// format of GDG command
	private static final String GRANT_FORMAT = "GRANT {0} TO {1}";

	private static final MessageFormat FORMAT = new MessageFormat(GRANT_FORMAT);

	/**
	 * @param commandLine
	 * @throws ParseException
	 */
	public Grant(String commandLine) throws ParseException {
		super(commandLine);
		// parse command
	...
	public void execute() throws Exception {
		if ((getRoles() != null) && (getPermissions() != null)){
			...
			util.grant(JobId.VALUE, perms, roles);
			
			for (int k=0; k<roles.length; k++){
				for (int i=0; i<perms.length; i++){
					System.out.println(AntUtilMessage.JEMZ022I.toMessage().getFormattedMessage(perms[i], roles[k]));
				}
			}
		}
