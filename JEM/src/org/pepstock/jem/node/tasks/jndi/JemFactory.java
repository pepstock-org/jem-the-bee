...
public class JemFactory implements ObjectFactory {
	...
	public Object getObjectInstance(Object object, Name name, Context ctx, Hashtable<?, ?> env) throws JNDIException {
		...
		for (int i = 0; i < JemResource.PROPERTIES_ALL.size(); i++) {
			String propertyName = JemResource.PROPERTIES_ALL.get(i);
	...
	public static Object createRestClient(Properties properties) throws JNDIException {
		String urlString = properties.getProperty(JemResource.URL);
		if (urlString == null){
			throw new JNDIException(NodeMessage.JEMC136E, JemResource.URL);
		}

		String username = properties.getProperty(JemResource.USERID);
		if (username == null){
			throw new JNDIException(NodeMessage.JEMC136E, JemResource.USERID);
		}

		String password = properties.getProperty(JemResource.PASSWORD);
		if (password == null){
			throw new JNDIException(NodeMessage.JEMC136E, JemResource.PASSWORD);
		}
